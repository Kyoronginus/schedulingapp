...
Future<User> getCurrentUser() async {
  try {
    ...
    if (userData == null) {
      debugPrint(
        '❌ AuthService: User not found in database for ID: $idToQuery',
      );
    ...
    debugPrint(
      '✅ AuthService: Successfully found and created User object for ID: ${user.id}',
    );
    return user;
  } on AuthException catch (e) {
    ...
    if (isAccountLinkingSignal) {
      ...
      try {
        ...
        if (context.mounted) {
          _handleOAuthError(
            context,
            retryError as AmplifyException,
            'Facebook',
          );
        }
        return false;
    ...
    if (session.isSignedIn) {
      ...
      for (final attr in attributes) {
        if (attr.userAttributeKey.key == 'name' ||
          ...
          debugPrint(
            '📋 Updated attribute: ${attr.userAttributeKey.key} = ${attr.value}',
          );
        }
      }
